...
export class ItemsPage implements OnInit {
      ...
      if (this.router.getCurrentNavigation().extras.state) {
        ...
        this.translate.get("loading").subscribe(value => {
          ...
          promise.then((value) => {
            ...
            this.loadProducts(this.subcategory_id, this.pageNo); //OUTPUT Task will be done in 1 second
          }, (error) => {
            console.log(error);
          });
        });
        //this.RefreshCategories(this.category_id);
  ...
  segmentChanged(event) {
    ...
    this.pageNo = 1;
    // this.translate.get("loading").subscribe(value => { console.log("value: "+ value); this.uiElementService.presentLoading(value); this.loadProducts(this.category_id); });
  }

  loadSubCategories(category_id: number) {
    return new Promise((resolve, reject) => {
      this.subscriptions.push(this.apiService.getCategoriesSub(category_id).subscribe(res => {
  ...
  loadProducts(category_id: number, pageNo: number) {
    ...
    this.subscriptions.push(this.apiService.getProductsWithCategoryId(Constants.SCOPE_ECOMMERCE, category_id, pageNo).subscribe(res => {
  ...
  addProCart(pro: Product) {
    ...
    //  {
    //   //pro.savings=pro.price-pro.sale_price;
  ...
  navReviews(pro) {
    let navigationExtras: NavigationExtras = { state: { product: pro } };
